...
public class CollectorsCommand implements CommandExecutor {
  ...
  public boolean onCommand(final CommandSender sender, final Command command, final String s, final String[] args) {
    if (!sender.hasPermission("collectors.admin")) {
      return false;
    }
      ...
      case "remove": {
        if (sender instanceof Player) {
          ...
          if (Collector.chunkHasCollector(location)) {
            ...
            return Style.message(sender,
                                 ChatColor.RED.toString() + "Collector removed from x: " + location
        ...
        if (Collector.chunkHasCollector(location)) {
          if (Collector.isCollector(location.subtract(new Vector(0, 0.5, 0)).getBlock().getLocation())) {
            final Collector collector =
              Collector.get(location.subtract(new Vector(0, 0.5, 0)).getBlock().getLocation());

            collector.getAmounts().entrySet().stream().filter(entry -> collectionType == entry.getKey())
              .forEach(entry -> entry.setValue(amount));
            return Style.message(sender, ChatColor.RED.toString() + "Collection type " + ChatColor.YELLOW
              .toString() + collectionType.toString() + ChatColor.RED
                                           .toString() + " has been set to " + ChatColor.YELLOW
                                           .toString() + amount + ChatColor.RED.toString() + ".");
          } else {
